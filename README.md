...
```
Sources (OpenAlex, PubMed, DBLP, SSRN, Scholarly Commons, OpenBU, NIH, NSF, arXiv, CrossRef, bioRxiv)
...
### Harvesting
Papers are collected from 11 sources. OpenAlex is the primary source, queried by BU's ROR identifier for exact institutional matching. DBLP provides CS conference proceedings via XML dump processing with two-tier name matching and OpenAlex verification. Other sources use a mix of affiliation text search, MeSH terms, faculty name matching, and direct repository access. Each source has known biases: OpenAlex undercounts conference proceedings, PubMed only covers biomedical literature, SSRN skews toward law and business, DBLP only covers CS venues. We cast a wide net and filter downstream rather than relying on any single source.
...
|--------|----------|----------------|-------------------|
| **OpenAlex** | 15,453 | Primary source, 250M+ works | BU ROR ID (exact) |
| **OpenBU** | 1,470 | Theses, dissertations, tech reports | Native (all BU) |
| **PubMed** | 1,460 | Biomedical AI work | Affiliation field |
...
Papers often appear in multiple sources, so total mentions exceed the 11,870 deduplicated paper count.
...
|- source_dblp.py             # DBLP (CS conferences, API-based)
...
|- harvest_dblp_dump.py       # DBLP XML dump processor (bulk backfill)
|
...
|- Data
|- data/
|   |- sonnet_classification_bu_verified.json  # Master dataset (11,870 papers)
