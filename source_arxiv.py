...
def _search_arxiv(query: str, max_results: int = 2000) -> list[dict]:
...
def harvest() -> list[dict]:
    ...
    query1 = f'all:"Boston University" AND ({cat_filter})'
    logger.info(f"  arXiv query 1: affiliation-based")
    papers1 = _search_arxiv(query1, max_results=5000)
    for p in papers1:
        aid = p.get("source_id", "")
    ...
    for group in bu_groups:
        query = f'all:{group} AND ({cat_filter})'
        logger.info(f"  arXiv query: {group}")
        papers = _search_arxiv(query, max_results=500)
        new = 0
        for p in papers:
            aid = p.get("source_id", "")
            if aid not in seen_ids:
